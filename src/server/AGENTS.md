...
- **Cancelled ≠ disconnected:** an explicit cancel stops the workflow; a client disconnect lets it keep running in the background (`BackgroundTaskManager` keeps executing; the run's `in_progress` ledger row is the durable record), and a reconnect resumes the live stream.
...
- **Treating local executor state as truth** — `BackgroundTaskManager` handles, "is the task in my registry" liveness. Liveness = the advisory lock; status = the ledger row.
...
- **Read-modify-write on shared rows without a CAS/guarded UPDATE or the right advisory lock.** Terminal transitions go through `TurnCoordinator`'s single finalize; never a second writer.
