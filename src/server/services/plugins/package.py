...
def _scrub_entry_secrets(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(args, list):
        # ``--flag=SECRET`` is the shape the extractor lifts; a bare
        # ``--token SECRET`` pair is left alone on both sides, since neither
        # can tell it from a benign positional.
        scrubbed_args = []
        for arg in args:
            flag, sep, value = (
...
def validate_package(raw: bytes, *, subdir: str | None = None) -> ValidatedPackage:
    ...
    """
    tree = extract_plugin_archive(raw)
    candidates = discover_plugin_roots(tree)
    if not candidates:
        raise PluginFatal(
    ...
    extension = parse_extension(manifest_extension(manifest, NAMESPACE))
    # Validate only. The refs themselves are written by the async install and
    ...
    resolve_binds(extension, entry_plans)
