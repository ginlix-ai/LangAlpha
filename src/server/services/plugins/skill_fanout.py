...
async def fan_out_skills(
    ...
    # Names and command seeds are per-scope facts, so they come from this
    # scope's rows; the cap is not, so it gets its own count.
    held = await count_user_skills(user_id)
    ...
    def _over_cap(plan: SkillPlan, *, name: str = "", reason: str = "") -> None:
        # The default speaks for the count cap, which is the one this module
        # checks itself. A cap the write raised carries its own sentence,
        # because the storage cap is a different limit with a different remedy.
        reason = reason or _CAP_REASON
        report.components.append(
            ComponentResult.of(plan, "error", name=name, reason=reason)
        )
        report.diagnostics.append(
            Diagnostic(
                level="error", scope="skill", target=plan.dir,
                code="cap_reached", message=reason,
            )
        )
    ...
    for plan, outcome in zip(plans, outcomes, strict=True):
        ...
        # `held` grows with every create below, so this counts the budget this
        # install has actually spent rather than the plans it started with: a
        # plan the prepare phase rejected never took a slot. Same shape as
        # run_mcp_import's cap check, and it keeps the ValueError below meaning
        # what its name says instead of doubling as the cap's messenger.
        if held >= MAX_SKILLS_PER_USER:
            await drop_archive_if_unused(user_id, outcome.archive_key)
        ...
        try:
            ...
            continue
        except SkillCapReached as e:
            # Either the count cap lost to a concurrent write, or the storage
            # cap, which nothing above checks — a plugin whose skills are
            # simply too large arrives here on the ordinary path and used to be
            # told it had a trigger conflict. The count cap keeps this module's
            # own wording either way, so one limit reads as one limit.
            await drop_archive_if_unused(user_id, outcome.archive_key)
            _over_cap(
                plan, name=validated.name,
                reason="" if e.limit == "count" else str(e),
            )
            continue
        except ValueError as e:
            # A raced trigger conflict. This skill only.
            await drop_archive_if_unused(user_id, outcome.archive_key)
