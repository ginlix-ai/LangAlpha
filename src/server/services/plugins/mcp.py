...
Two deliberate v1 policy deviations, both legal per-entry skips reported
with ``spec_ref``: stdio commands are bound by ``ALLOWED_COMMANDS`` (bundled
``./`` executables and arbitrary binaries don't run), and entries that need
the plugin tree materialized (``cwd``, ``${PLUGIN_ROOT}``/``${PLUGIN_DATA}``)
are held back until the sandbox materializes plugin roots.
"""
...
from src.server.models.mcp_server import ALLOWED_COMMANDS, coerce_mcp_name
...
from src.server.services.plugins.schemas import MCP_SCHEMA, SCHEMA_URL_RE
...
def _plan_entry(key: str, entry: dict[str, Any]) -> McpEntryPlan:
    ...
    if entry.get("cwd") is not None or _mentions_plugin_vars(entry):
        return _skip(
            plan, "plugin_tree_unsupported",
            "entries that need the plugin tree materialized (cwd, "
            "${PLUGIN_ROOT}, ${PLUGIN_DATA}) are not supported yet",
        )
    ...
    if plan.transport == "stdio":
        ...
        if command not in ALLOWED_COMMANDS:
            return _skip(
                plan, "policy",
                f"command {command!r} is not allowed; this deployment runs "
                f"only {sorted(ALLOWED_COMMANDS)}",
            )
        plan.config = {
            "name": name,
            "transport": "stdio",
    ...
    for values in (
        ...
        if any(isinstance(v, str) and "${vault:" in v for v in values):
...
def validate_mcp_document(
    ...
    for error in _VALIDATOR.iter_errors(doc):
        ...
        if len(path) >= 2 and path[0] == "mcpServers":
            entry_errors.setdefault(str(path[1]), []).append(error.message)
        else:
            loc = ".".join(str(p) for p in path) or "mcp.json"
            return _component_error(
                "schema_invalid", f"{loc}: {error.message}"
            )
    ...
    for key, entry in (doc.get("mcpServers") or {}).items():
        if key in entry_errors:
            ...
            # oneOf failures enumerate every branch; the first message names
            # the closest one and the rest are noise on the wire.
