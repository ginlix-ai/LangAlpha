...
def discover_plugin_roots(files: dict[str, bytes]) -> list[PluginCandidate]:
...
def adapt_dialect(
    ...
    """Rewrite a vendor-dialect subtree into canonical shape, in memory.
    ...
    diagnostics = [
        Diagnostic(
            ...
            message=(
                f"adapted from the {vendor_dir}/plugin.json layout into the "
                "Agent Plugins canonical form"
            ),
        )
    ]

    manifest_raw = out.pop(f"{vendor_dir}/plugin.json")
    ...
    _canonical_schema(manifest, PLUGIN_SCHEMA)
    if isinstance(manifest.get("author"), str):
        manifest["author"] = {"name": manifest["author"]}

    refs: dict[str, list[dict[str, str]]] = {}
    if ".mcp.json" in out and "mcp.json" not in out:
        mcp_raw = out.pop(".mcp.json")
        mcp_doc: dict[str, Any] | None = None
        try:
            parsed = json.loads(mcp_raw.decode("utf-8"))
            if isinstance(parsed, dict):
                mcp_doc = parsed
        except (json.JSONDecodeError, UnicodeDecodeError):
            pass
        if mcp_doc is None:
            out["mcp.json"] = mcp_raw
        else:
            _canonical_schema(mcp_doc, MCP_SCHEMA)
