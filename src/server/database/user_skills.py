...
class SkillCapReached(ValueError):
    """A write would take the user past their skill count or storage cap.

    Same reasoning as :class:`SkillNameTaken`: a ValueError for every existing
    caller, a distinct type so a cap the install lost to a concurrent write is
    reported as a cap rather than as a trigger conflict. ``limit`` says which
    of the two, since they have different remedies and a caller with its own
    wording for one of them needs to know which one it caught.
    """
...
#
# This bounds row count, not resource use: MAX_SKILL_TOTAL_BYTES_PER_USER
# bounds storage and the sandbox sync, and the prompt cost is one manifest line
# per *enabled* skill, which the user controls per skill. Sized against what
# the ecosystem publishes rather than against a round number: single plugins
# routinely carry thirty-odd skills, so a limit in the dozens is spent by two
# installs and reads as the feature being broken.
...
async def count_user_skills(user_id: str) -> int:
    """How many rows the per-user count cap is enforced against.

    ``list_user_skills`` answers a different question, one scope's rows, so its
    length under-counts by every workspace-scoped skill the account holds. A
    caller budgeting against that number believes it has slots the write will
    refuse: an account of 6 user-tier and 44 workspace skills reads as 6, and
    every skill of the installing plugin is then rejected one at a time by a
    limit it was told it was nowhere near.
    """
    async with get_db_connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT COUNT(*) FROM user_skills "
                f"WHERE user_id = %s AND {_LIVE_SCOPE}",
                (user_id,),
            )
            row = await cur.fetchone()
