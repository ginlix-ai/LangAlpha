...
# stockId 缓存（避免重复请求）
_HKEX_STOCKID_CACHE: dict[str, int | None] = {}


def _hk_lookup_stock_id(stock_code: str) -> int | None:
    """通过 HKEx prefix.do 查找股票代码对应的 HKEx 内部 stockId。

    ⚠️ 关键：HKEx 的 titleSearchServlet 不能用股票代码作 keyword 搜（keyword 是
    全文检索，匹配不到时会返回任意公告）。必须用 stockId 这个内部 ID 精确查询。

    Args:
        stock_code: 5 位数字港股代码（如 '02498'）

    Returns:
        stockId（如 1000211551）或 None（找不到）
    """
    if stock_code in _HKEX_STOCKID_CACHE:
        return _HKEX_STOCKID_CACHE[stock_code]

    # prefix.do 返回 callback({...}) 形式的 JSONP
    # 用代码（去前导零，如 "2498"）查更准确
    code_no_pad = stock_code.lstrip("0") or "0"
    url = (
        f"{HKEX_BASE}/search/prefix.do"
        f"?callback=cb&lang=EN&type=A&name={code_no_pad}&market=SEHK"
    )
    ...
    try:
        ...
        stocks = data.get("stockInfo") or []
        for s in stocks:
            if s.get("code") == stock_code:
                sid = int(s.get("stockId"))
                _HKEX_STOCKID_CACHE[stock_code] = sid
                log.info("  [HKEx] %s → stockId=%d (%s)", stock_code, sid, s.get("name", ""))
                return sid
        log.warning("  [HKEx] 未找到股票代码 %s 的 stockId", stock_code)
    except Exception as e:
        log.warning("  [HKEx] 查询 stockId 失败: %s", e)

    _HKEX_STOCKID_CACHE[stock_code] = None
    return None
...
def _hkex_search(stock_code: str, t1code: str = "-2", page_size: int = 20, keyword: str = "") -> list[dict]:
    ...
    ⚠️ 修复说明：
    必须先用 _hk_lookup_stock_id() 解析出真正的 HKEx 内部 stockId，
    再用 stockId 精确查询。早期版本用 keyword=股票代码 全文检索，
    会因为 HKEx 的 keyword 是全文匹配而返回错误公司的公告。
    ...
    """
    # Step 1: 解析 stockId
    stock_id = _hk_lookup_stock_id(stock_code)
    if stock_id is None:
        log.warning("  [HKEx] 无法解析 stockId（%s），跳过搜索避免下载错误公司的文件", stock_code)
        return []

    # ⚠️ 关键：必须加 fromDate / toDate 日期范围，否则 HKEx 只返回最近约 8 条公告
    # 默认查询过去 6 年（覆盖 4-5 个完整财年）
    today = datetime.now()
    from_date = (today.replace(year=today.year - 6)).strftime("%Y%m%d")
    ...
    params = {
        ...
        "t2Gcode": "-2",
        "t2code": "-2",
        "stockId": str(stock_id),     # ← 关键 1：用真正的 stockId
        "from": "0",
        "fromDate": from_date,        # ← 关键 2：日期范围
        "toDate": to_date,
        "pageSize": str(page_size),
        "lang": "EN",
    }
    # 不再传 keyword（避免全文检索污染）
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            if result_str and result_str != "null":
                ...
                # ⚠️ 防御性二次过滤：保留 STOCK_CODE 包含目标代码的记录
                # （stockId 已经精确，理论上 100% 匹配；这里再过滤一次防止 HKEx
                # 偶尔返回相关联的衍生品记录）
                #
                # 注意：腾讯等大公司的 STOCK_CODE 可能形如 "00700<br/>80700"
                # （H 股 + 美元结算柜台），不能要求完全相等，要求包含即可。
                strict = []
                for item in items:
                    raw_code = (item.get("STOCK_CODE", "") or "").strip()
                    # 拆分 <br/> 分隔的多代码，看目标 stock_code 是否在其中
                    parts = re.split(r"<br\s*/?>", raw_code)
                    parts = [p.strip() for p in parts if p.strip()]
                    if stock_code in parts:
                        strict.append(item)
                if strict:
                    return strict
                # ⚠️ 严格匹配为空 → 返回空列表（不再回退到任意 items），避免下载错误公司的文件
                if items:
                    sample = (items[0].get("STOCK_CODE", "") or "")[:50]
                    log.warning("  [HKEx] stockId=%d 返回 %d 条但 STOCK_CODE 都不含 %s（首条 STOCK_CODE=%s）",
                                stock_id, len(items), stock_code, sample)
                return []
            return []
    except Exception as e:
...
def _hkex_filter_by_title(stock_code: str, keywords: list[str], max_pages: int = 3,
                          page_size: int = 100) -> list[dict]:
    """从 HKEx 全部公告中按标题关键词过滤特定文档类型（年报/中期报告/季报）。

    ⚠️ 背景：HKEx 现在的 t1code 精确分类参数（40000/40100/40200）实际上常常失效，
    返回的是混杂的最新公告而非真正的年报/中期报告。最稳妥的做法是用 t1code=-2
    取全部公告 + 按标题关键词过滤。

    Args:
        stock_code: 5 位港股代码
        keywords: 标题需要包含的关键词列表（OR 关系，大小写不敏感）
        max_pages: 最多翻多少页
        page_size: 每页大小
    ...
    for page in range(max_pages):
        ...
        # _hkex_search 内部只用了 from=0；直接重写一遍带 from=offset
        stock_id = _hk_lookup_stock_id(stock_code)
        if stock_id is None:
            return []
        ...
        url = f"{HKEX_BASE}/search/titleSearchServlet.do?{urllib.parse.urlencode(params)}"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 LangAlpha/1.0"})
        try:
            with urllib.request.urlopen(req, timeout=30) as resp:
                data = json.loads(resp.read().decode("utf-8"))
            result_str = data.get("result", "null")
            items = json.loads(result_str) if result_str and result_str != "null" else []
            if not items:
                break
            new_count = 0
            for item in items:
                # 防御：STOCK_CODE 必须包含目标代码
                raw_code = (item.get("STOCK_CODE", "") or "").strip()
                parts = re.split(r"<br\s*/?>", raw_code)
                parts = [p.strip() for p in parts if p.strip()]
                if stock_code not in parts:
                    continue
                # 去重
                fid = item.get("FILE_LINK") or item.get("FILE_ID") or item.get("TITLE", "")
                if fid in seen_ids:
                    continue
                seen_ids.add(fid)
                all_items.append(item)
                new_count += 1
            # 如果一页里没有新增，说明分页失效或到底了，提前退出
            if new_count == 0:
                break
        except Exception as e:
            log.warning("  [HKEx] 分页 %d 失败: %s", page, e)
            break
    ...
    filtered = [
        ...
        if any(kw in (it.get("TITLE", "") or "").upper() for kw in kws_upper)
    ]
    return filtered


def download_hk_financials(symbol: str, knowledge_dir: Path, catalog: CatalogManager, years: int = 3) -> int:
    """下载港股财报 PDF（年报 / 中期报告 / 季报，如有）。

    使用 HKEx titleSearchServlet.do API + 标题关键词过滤。
    ⚠️ HKEx t1code 精确分类已失效（2026 年起），改用全量公告 + 标题匹配。
    """
    ...
    # 标题关键词分类（OR 匹配）
    DOC_TYPE_KEYWORDS = [
        ("annual-report",  ["ANNUAL REPORT", "年度報告", "年度报告"]),
        ("interim-report", ["INTERIM REPORT", "HALF-YEAR REPORT", "中期報告", "中期报告"]),
        # H 股一般无季报，但部分内地企业 H 股或科技股有
        ("quarterly-report", ["QUARTERLY REPORT", "FIRST QUARTERLY", "THIRD QUARTERLY",
                              "季度報告", "季度报告", "Q1 REPORT", "Q3 REPORT"]),
    ]

    for doc_label, keywords in DOC_TYPE_KEYWORDS:
        items = _hkex_filter_by_title(stock_code, keywords, max_pages=4, page_size=100)
...
# 巨潮 orgId 缓存
_CN_STOCK_LIST_CACHE: list[dict] | None = None
_CN_ORGID_CACHE: dict[str, tuple[str, str]] = {}  # code → (orgId, column)


def _cn_lookup_org_id(stock_code: str) -> tuple[str | None, str | None]:
    """通过巨潮股票列表 API 查找股票的 orgId 和 column。

    ⚠️ 关键：巨潮的 orgId 不能用 'gs{exchange}{code}' 拼接，必须查表：
      - 五粮液 000858 → orgId=gssz0000858（11 位）
      - 茅台 600519 → orgId=gssh0600519（11 位）
      - 宁德时代 300750 → orgId=GD165627（完全不规则）

    Returns:
        (orgId, column) 或 (None, None)
    """
    global _CN_STOCK_LIST_CACHE

    if stock_code in _CN_ORGID_CACHE:
        return _CN_ORGID_CACHE[stock_code]

    # 加载股票列表（一次性）
    if _CN_STOCK_LIST_CACHE is None:
        try:
            url = "http://www.cninfo.com.cn/new/data/szse_stock.json"
            req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
            with urllib.request.urlopen(req, timeout=30) as r:
                d = json.loads(r.read().decode("utf-8"))
            _CN_STOCK_LIST_CACHE = d.get("stockList") or []
            log.info("  [巨潮] 加载股票列表 %d 只", len(_CN_STOCK_LIST_CACHE))
        except Exception as e:
            log.warning("  [巨潮] 加载股票列表失败: %s", e)
            _CN_STOCK_LIST_CACHE = []

    # 在列表中查找
    for s in _CN_STOCK_LIST_CACHE:
        if s.get("code") == stock_code:
            org_id = s.get("orgId", "")
            # column 由代码前缀决定
            if stock_code.startswith("6") or stock_code.startswith("9"):
                ...
                column = "szse"  # 深市主板/创业板
            elif stock_code.startswith(("8", "4")):
                column = "bj"  # 北交所
            else:
                column = "szse"
            log.info("  [巨潮] %s %s → orgId=%s, column=%s", stock_code, s.get("zwjc", ""), org_id, column)
            _CN_ORGID_CACHE[stock_code] = (org_id, column)
            return org_id, column

    log.warning("  [巨潮] 未找到股票代码 %s", stock_code)
    _CN_ORGID_CACHE[stock_code] = (None, None)
    return None, None
...
def download_cn_financials(symbol: str, knowledge_dir: Path, catalog: CatalogManager, years: int = 3) -> int:
    ...
    org_id, column = _cn_lookup_org_id(stock_code)
    count = 0

    if not org_id:
        log.warning("[A股财报] 无法解析 orgId（%s），跳过下载", stock_code)
        return 0

    log.info("[A股财报] 正在获取 %s 的财报列表...", symbol)

    # 巨潮资讯网 API
    # category 列表（实测 2026 年最新有效分类）：
    ...
    #   ⚠️ A 股没有"二季报"——半年报取代；没有"四季报"——年报取代
    categories = [
        ("category_ndbg_szsh",   "annual-report"),
        ("category_bndbg_szsh",  "semi-annual-report"),
        ("category_yjdbg_szsh",  "q1-report"),
        ("category_sjdbg_szsh",  "q3-report"),
    ]

    # 构造日期范围（过去 years+1 年）
    today = datetime.now()
    se_date = f"{today.year - years - 1}-01-01~{today.strftime('%Y-%m-%d')}"
    ...
    for cat_code, cat_label in categories:
        try:
            ...
            post_data = urllib.parse.urlencode({
                ...
                "tabName": "fulltext",
                "pageSize": str(max(years * 2, 10)),
                "pageNum": "1",
                "column": column,                  # ← 关键：用 'sse' / 'szse' / 'bj'，不是 'SHSE_MAIN'
                "category": cat_code,
                "seDate": se_date,                 # ← 关键：必须给日期范围
                "isHLtitle": "true",
            }).encode("utf-8")
            ...
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                log.info("  [%s] 找到 %d 条记录", cat_label, len(announcements))

                # 仅下载真正的"XX年年度报告"（剔除"摘要"/"更正"/"取消"等）
                main_anns = [
                    a for a in announcements
                    if "摘要" not in (a.get("announcementTitle") or "")
                    and "取消" not in (a.get("announcementTitle") or "")
                    and "更正" not in (a.get("announcementTitle") or "")
                ]

                for ann in main_anns[:years]:
...
def download_cn_announcements(symbol: str, knowledge_dir: Path, catalog: CatalogManager, limit: int = 20) -> int:
    ...
    if not org_id:
        log.warning("[A股公告] 无法解析 orgId（%s），跳过下载", stock_code)
        return 0

    log.info("[A股公告] 正在获取 %s 的公告列表...", symbol)

    # 必须给日期范围（巨潮 API 默认只返回最近）
    today = datetime.now()
    se_date = f"{today.year - 2}-01-01~{today.strftime('%Y-%m-%d')}"
...
def download_transcripts(symbol: str, knowledge_dir: Path, catalog: CatalogManager, years: int = 2) -> int:
    ...
    for year in target_years:
        try:
            data = _fmp_get_v4(f"batch_earning_call_transcript/{symbol}", {"year": str(year)})
            if isinstance(data, list) and data:
                log.info("  [%d] 找到 %d 个季度", year, len(data))
                all_transcripts.extend(data)
            else:
                log.info("  [%d] 0 个季度", year)
        except Exception as e:
            log.warning("  [%d] FMP 请求失败: %s", year, e)
    ...
    for t in all_transcripts:
        ...
        year = t.get("year", date[:4] if date else "")
        ...
        if not dest.exists():
            ...
            log.info("  [ok] %s (%d chars)", filename, len(content))
        else:
...
def main():
    ...
    if args.all or args.financials:
        ...
        if market == "hk":
            ...
            # 10-K = 年报（每年 1 份），10-Q = 季报（每年 3 份）
            # ADR 公司用 20-F（年报）+ 6-K（半年报）
            US_FILING_TYPES = [
                ("10-K", "annual-report"),
                ("10-Q", "quarterly-report"),
                ("20-F", "annual-report-adr"),  # ADR 年报
            ]
            financials_dir = _ensure_dir(knowledge_dir / "financials")
            us_count = 0
            for filing_type, doc_label in US_FILING_TYPES:
                try:
                    # 季报多拉一些（每年 3 份），年报少拉
                    limit = args.years * 4 if filing_type == "10-Q" else args.years + 1
                    filings = _fmp_get(f"sec_filings/{symbol}", {"type": filing_type, "limit": str(limit)})
                    if not filings:
                        continue
                    log.info("  [%s/%s] 找到 %d 份", filing_type, doc_label, len(filings))
                    for filing in filings:
                        link = filing.get("finalLink") or filing.get("link", "")
                        date = filing.get("fillingDate", "")[:10] or "unknown"
                        filename = f"{date}_{filing_type}.pdf"
                        dest = financials_dir / _safe_filename(filename)
                    ...
                    log.warning("[美股财报 %s] 失败: %s", filing_type, e)
            log.info("[美股财报] 完成，下载 %d 份文件", us_count)
            total += us_count
