...
async def test_recovered_task_run_gets_its_retention_stamp_before_finalize():
    ...
    with (
        patch(
            "src.server.services.subagent_run_coordinator.SubagentRunCoordinator",
            return_value=ledger,
        ),
        patch(
    ...
    # Set-if-absent: the recovery stamp must not resurrect a collector's
    # shorter retention window.
    assert all(c.kwargs.get("nx") is True for c in pipe.expire.call_args_list)
