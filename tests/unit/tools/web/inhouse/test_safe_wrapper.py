...
from src.tools.web.breaker import CircuitState, CircuitBreaker
from src.tools.web.inhouse.backend import CrawlOutput
from src.tools.web.inhouse.safe_wrapper import (
    SafeCrawlerWrapper,
...
class TestThreeLayerHealth:
    ...
    async def test_blocked_cache_lru_evicts_oldest(self):
        from src.tools.web.inhouse.safe_wrapper import _BLOCKED_LRU_CAP
    ...
    async def test_host_breakers_lru_evicts_oldest(self):
        ...
        for i in range(_HOST_BREAKERS_LRU_CAP):
            wrapper._host_breakers[f"host{i}.example"] = CircuitBreaker()
...
class TestOpportunisticReaper:
    ...
    async def test_reap_runs_again_after_interval(self):
        from src.tools.web.inhouse.safe_wrapper import _REAPER_INTERVAL_SECONDS
...
class TestCircuitBreakerTransitions:
    ...
    async def test_record_failure_triggers_reset_callback(self):
        ...
        await cb.record_failure(on_open=mock_reset)
...
class TestWrapperStatus:
    ...
    def test_is_healthy_indifferent_to_host_breakers(self):
        ...
        wrapper = _make_wrapper()
        wrapper._host_breakers["reuters.com"] = CircuitBreaker()
